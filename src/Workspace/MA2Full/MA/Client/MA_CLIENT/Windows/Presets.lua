...
function view.new(position, size)
	...
	self.frame = script.Presets:Clone()
...
function view:redraw()
	...
	if difference > 0 then
		for i=1, difference do
			...
			button.MouseButton1Click:Connect(function()
				Events.Pools.Presets.SelectPresets:FireServer(tonumber(button.Number.Text))
			end)
			
			button.MouseButton2Click:Connect(function()
				-- Create group
				print("Store Presets")
				Events.Pools.Presets.StorePresets:FireServer(tonumber(button.Number.Text), button.Number.Text, {255, 255, 255})
			end)
	...
	for k, button in pairs(self.buttons) do
		...
		button.PresetName.Text = "EMPTY"
		button.PresetName.TextColor3 = Color3.fromRGB(102, 102, 102)
		if self.groupMap[tonumber(id)] then
			local curGroup = self.groupMap[tonumber(id)]
			button.PresetName.Text = curGroup.presetName
			button.PresetName.TextColor3 = Color3.fromRGB(255, 255, 255)
...
function view:init()
	...
	Events.Pools.Presets.GetPresets.OnClientEvent:Connect(function(groups)
		self.groups = groups
		for _, v in pairs(self.groups) do
			self.groupMap[tonumber(v.PresetId)] = v
		end
		self:redraw()
	end)
